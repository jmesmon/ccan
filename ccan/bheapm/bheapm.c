 ...
 *    +   +			  +
 *
 * N=4  + D=3 log2(N+1) = 2.321   +(0, points into the struct bheapm_)
 *     / \    log2(N) = 1.386	  |
 *    +   +			  +(1)
 *   /				  |
 *  +				  +(2)
 ...
 *
 */
static struct bheapm_node **swap_node_with_parent(struct bheapm_node *a, struct bheapm_node *b,
		struct bheapm_node **a_slot)
{
	/* direction of b from a, soon to be direction of a from b */
	bool d_of_b = a->d[0] != b;
	struct bheapm_node b_links = *b;
	assert(a->d[d_of_b] == b);
	assert(*a_slot == a);
	*a_slot = b;
	b->d[ d_of_b] = a;
...
static void poison_parent(struct bheapm_node **slot, int depth_mark)
{
	*slot = (void *)(uintptr_t)(BH_POISON_PARENT + depth_mark);
}

static void poison_parent_stack(struct bheapm_node **parent_stack, int depth)
{
...
#else
static inline void poison_parent(struct bheapm_node **slot, int dm)
{
	(void)slot;
	(void)dm;
}

static inline void poison_parent_stack(struct bheapm_node **ps, int depth)
{
	(void)ps;
	(void)depth;
}
 ...
 * B[0] = A (instead of B)
 */
static void swap_node_up(struct bheapm_node *c, struct bheapm_node **parent_stack, int depth)
{
	struct bheapm_node *parent = parent_stack[depth];
	struct bheapm_node *parent_2 = parent_stack[depth - 1];
	swap_node_with_parent(parent, c, &parent_2->d[parent_2->d[0] != parent]);
	poison_parent(parent_stack + depth, depth);
}

static int bheapm_ord(struct bheapm_ *bh, struct bheapm_node *a, struct bheapm_node *b)
{
	return bh->order(bh, a, b);
}
 ...
 */
static void bubble_up(struct bheapm_ *bh, struct bheapm_node *c,
		struct bheapm_node **parent_stack, int depth)
{
	int d;
	for (d = depth - 1; d; d--) {
		/* c >= it's parent */
		if (bheapm_ord(bh, c, parent_stack[d]) >= 0)
			break;
		swap_node_up(c, parent_stack, d);
	}
}

void bheapm_push_(struct bheapm_ *bh, struct bheapm_node *new_node)
{
	/* the depth where the new node (+1) will be inserted */
	int future_tree_depth = bheapm_depth(bh->sz + 1);
	 ...
	 * bheapm)] member named 'top' [which has type (struct
	 * bheapm_node *)] is actually a (struct bheapm_node *)
	 * itself.
	 *
	 * This relys on code checking ->d[0] first, and not checking ->d[1]
	 * unless that fails (For parent_stack[0], ->d[1] actually looks at the
	 * fields in struct bheapm_ that follow ->top. */
	parent_stack[0] = &((union bheapm_union *)&bh->top)->node;
	...
	*c = new_node;
	...
	bubble_up(bh, new_node, parent_stack, future_tree_depth);
}
...
static void push_down(struct bheapm_ *bh)
{
	...
	for (;;) {
		...
		if (!c->d[0]) {
			...
			bool p_gt_l = bheapm_ord(bh, c, c->d[0]) > 0;
			bool p_gt_r = bheapm_ord(bh, c, c->d[1]) > 0;
			...
			if (!p_gt_l) {
				...
				if (!p_gt_r)
					...
					bool l_gt_r = bheapm_ord(bh, c->d[0], c->d[1]) > 0;
					parent_slot = swap_node_with_parent(c,
...
static inline struct bheapm_node **bheapm_get_last_node_slot(struct bheapm_ *bh)
{
	if (!bh->sz)
		return NULL;

	/* the depth of the tree prior to removing a node */
	int depth = bheapm_depth(bh->sz);
...
{
	bhn **c = bheapm_get_last_node_slot(bh);
	if (!c)
		return NULL;
