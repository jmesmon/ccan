...
static void p_ns(const char *n, struct t2 t)
{
	printf("%17s : %8"PRIu64" %8"PRIu64" %8"PRIu64"\n",
		...
		time_to_nsec(t.t[0]),
		time_to_nsec(t.t[1]),
		time_to_nsec(time_add(t.t[0], t.t[1])));
}
...
static void pp_N(int len)
{
	...
	for (i = 0; i < r; i++) {
		...
		a[0].t[0] = time_add(a[0].t[0], r.t[0]);
		a[0].t[1] = time_add(a[0].t[1], r.t[1]);
	}
	...
	for (i = 0; i < r; i++) {
		...
		a[1].t[0] = time_add(a[1].t[0], r.t[0]);
		a[1].t[1] = time_add(a[1].t[1], r.t[1]);
	}
