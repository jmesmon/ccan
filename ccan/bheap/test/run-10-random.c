...
#include <ccan/bheap/bheap_print.h>
...
static void poison_node(struct binary_heap_node *b, int i)
{
...
{
	struct binary_heap bh = BHEAP_INIT(X_ord);
	...
	int max = INT_MIN;
	for (i = 0; i < ARRAY_SIZE(x); i++) {
		//bheap_print_horz(bh.top, 0, X_to_string, stdout);
		struct X *f = container_of(bheap_pop(&bh), struct X, bhn);
